...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    if use_recurrent_dropout:
      cell = cell_fn(hps.rnn_size, dropout=self.hps.recurrent_dropout_prob)
    else:
    ...
    self.input_x = tf.placeholder(dtype=tf.float32, shape=[None, None, INWIDTH])
    self.output_x = tf.placeholder(dtype=tf.float32, shape=[None, LENGTH, OUTWIDTH])
    
    
    actual_input_x = self.input_x
    #self.initial_state = cell.zero_state(batch_size=hps.batch_size, dtype=tf.float32)
    self.initial_state = cell.get_initial_state(batch_size=hps.batch_size, dtype=tf.float32)
    self.aux_state = tf.placeholder(tf.float32, [2, 300, 256])
  ...
  def load_json(self, jsonfile='./tf_rnn/rnn.json'):
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
  ...
  prev_x = np.zeros((1, 100, OUTWIDTH))
  prev_x = init_z
...
def rnn_output_size(mode):
  ...
  if (mode == MODE_ZC) or (mode == MODE_ZH):
    return (512+512) 
  return 512 # MODE_Z or MODE_Z_HIDDEN

def rnn_output(state, z, mode):
  if mode == MODE_ZCH:
    return np.concatenate([z, np.concatenate((state.c,state.h), axis=1)[0]])
  if mode == MODE_ZC:
    return np.concatenate([z, state[0]])
  if mode == MODE_ZH:
    return np.concatenate([z, state[1][0]]) #512 + 512 = 1024 EMB size + num cells
